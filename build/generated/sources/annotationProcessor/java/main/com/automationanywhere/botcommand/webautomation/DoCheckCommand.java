...
public final class DoCheckCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(DoCheckCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    if(parameters.containsKey("check") && parameters.get("check") != null && parameters.get("check").get() != null) {
      convertedParameters.put("check", parameters.get("check").get());
      if(convertedParameters.get("check") !=null && !(convertedParameters.get("check") instanceof Boolean)) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.UnexpectedTypeReceived","check", "Boolean", parameters.get("check").get().getClass().getSimpleName()));
      }
    }
    if(convertedParameters.get("check") == null) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.notEmpty","check"));
    }
    ...
    try {
      command.action((String)convertedParameters.get("sessionName"),(String)convertedParameters.get("search"),(String)convertedParameters.get("type"),(Boolean)convertedParameters.get("check"),(Number)convertedParameters.get("timeout"),(String)convertedParameters.get("attribute"));Optional<Value> result = Optional.empty();
