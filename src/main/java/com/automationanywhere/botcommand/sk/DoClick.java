...
@CommandPkg(label = "Click", name = "clickelement",
        ...
        node_label = "Click Element", icon = "pkg.svg", comment = true ,  text_color ="#777777" , background_color = "#777777" )
    ...
    public void action(@Idx(index = "1", type = TEXT) @Pkg(label = "Session name",  default_value_type = STRING, default_value = "Default") @NotEmpty String sessionName,
    				    ...
    				    ) throws Exception
     {
    	try {
    		...
    		utils.doClick(driver,jspath,library,timeout.intValue(),attribute);
    	}
    	catch (Exception e) {
    		throw new BotCommandException("CLICK "+jspath+" : "+e.getMessage());
    	}
