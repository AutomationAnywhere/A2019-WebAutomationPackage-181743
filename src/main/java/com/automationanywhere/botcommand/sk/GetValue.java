...
@CommandPkg(label = "Get Value", name = "getvalueelement",
        ...
        node_label = "Get Element Value", icon = "pkg.svg", comment = true ,  text_color ="#777777" , background_color = "#777777",
    ...
    public StringValue action(@Idx(index = "1", type = TEXT) @Pkg(label = "Session name",  default_value_type = STRING, default_value = "Default") @NotEmpty String sessionName,
    				   ...
    				   ) throws Exception
     {
		...
		try {
			...
			value = utils.getValue(driver,jspath,library,timeout.intValue(),attribute	);
		}
	    catch (Exception e) {
	   		throw new BotCommandException("GETVALUE "+jspath+" : "+e.getMessage());
	   	}
