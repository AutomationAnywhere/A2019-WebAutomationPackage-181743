...
@CommandPkg(label = "Send Key Strokes", name = "sendkeys",
        ...
        node_label = " to {{search}}", icon = "pkg.svg", comment = true , group_label="Set",  text_color = "#2F4F4F" , background_color =  "#2F4F4F")
    ...
    public void action(@Idx(index = "1", type = TEXT) @Pkg(label = "Session name",  default_value_type = STRING, default_value = "Default") @NotEmpty String sessionName,
    				   ...
    				   @Idx(index = "4", type = AttributeType.SELECT, options = {
    						    ...
    						    @Idx.Option(index = "4.2", pkg = @Pkg(label = "Credential", value = "CREDENTIAL"))})
    					           @Pkg(label = "Keys", description = "", default_value = "KEYS", default_value_type = STRING)
