...
@CommandPkg(label = "Get Current Session", name = "getcurrentsession",
        ...
        node_label = "id of {{sessionName}}", icon = "pkg.svg", comment = true ,group_label="Session", text_color = "#2F4F4F" , background_color =  "#2F4F4F",
    ...
    public StringValue action(@Idx(index = "1", type = TEXT) @Pkg(label = "Session name",  default_value_type = STRING, default_value = "Default") @NotEmpty String sessionName
    				   ) throws Exception
     {
		...
		try {
			...
			value = utils.getCurrentSessionID(driver,connection.getBrowserType());
		}
	    catch (Exception e) {
